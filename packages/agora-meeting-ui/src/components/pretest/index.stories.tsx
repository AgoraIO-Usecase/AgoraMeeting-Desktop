...
const cameraList = [...'.'.repeat(3)].map((item, index) => ({
  label: '摄像头' + (index + 1),
  deviceId: 'camera-' + (index + 1),
}));
const microphoneList = [...'.'.repeat(3)].map((item, index) => ({
  label: '麦克风' + (index + 1),
  deviceId: 'microphone-' + (index + 1),
}));
const speakerList = [...'.'.repeat(3)].map((item, index) => ({
  label: '扬声器' + (index + 1),
  deviceId: 'speaker-' + (index + 1),
}));
